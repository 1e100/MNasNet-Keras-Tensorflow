...
def get_params(model):
    ...
    print("Total parameters of the net: " + str(total_parameters) + " == " +
          str(total_parameters / 1000000.0) + "M")
...
def write_summary(tensor, writer, name):
    ...
    ):  # record_summaries_every_n_global_steps(1)
...
if __name__ == "__main__":
    ...
    dset_train = tf.data.Dataset.from_tensor_slices(
        (x_train, y_train_ohe)).shuffle(n_elements_train).batch(batch_size)
    dset_test = tf.data.Dataset.from_tensor_slices(
        (x_test, y_test_ohe)).shuffle(n_elements_test).batch(batch_size)
    ...
    init_model(
        model, input_shape=(batch_size, image_size, image_size, channels))
    ...
    )  # can use also ckpt = tfe.Checkpoint(model=model)

    restore_state(saver_model, 'weights/last_saver')

    train(
        dset_train=dset_train, dset_test=dset_test, model=model, epochs=epochs)
