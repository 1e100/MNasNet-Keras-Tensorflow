...
def conv_bn(x, filters, kernel_size, strides=1, alpha=1, activation=True):
    ...
    x = layers.Conv2D(
        ...
        padding='same',
        use_bias=False,
        kernel_regularizer=regularizers.l2(l=0.0003))(x)
...
def sepConv_bn_noskip(x, filters, kernel_size, strides=1):
    ...
    x = depthwiseConv_bn(
        x, depth_multiplier=1, kernel_size=kernel_size, strides=strides)
...
def MBConv_idskip(x_input,
                  filters,
                  kernel_size,
                  strides=1,
                  filters_multiplier=1,
                  alpha=1):
    """ Mobile inverted bottleneck convolution (Block b, c, d, e of MNasNet
    ...
    x = conv_bn(
        x_input,
        filters=depthwise_conv_filters * filters_multiplier,
        kernel_size=1,
        strides=1)
    x = depthwiseConv_bn(
        x, depth_multiplier=1, kernel_size=kernel_size, strides=strides)
